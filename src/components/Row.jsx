...
export default function Row({ row, rowIndex, hasCircuitBox, circuitboxId }) {
  return (
    <div className='aspect-video w-full border-2 p-1 relative flex items-center justify-center'>
      <span className='absolute top-0 left-1'>{row.value}</span>
      <div className='rounded-full w-4 h-4 bg-slate-800 border-2 border-white' id={row.arrowEnd} />
